...
// Verificar si se recibió el parámetro 'seccion'
if(isset($_GET['seccion'])) {
    // Obtener el valor de 'seccion' desde el parámetro de la URL
    $seccion = $_GET['seccion'];

    // Consultar la base de datos para obtener las noticias correspondientes a la sección
    $consulta = "SELECT * FROM noticias WHERE segment = ? ORDER BY updated_at DESC";
...
<head>
    ...
    <title>Segmento - <?php echo htmlspecialchars($seccion); ?></title>
...
<body>
    ...
    <main>
        <div class="in_container">
            ...
            <div class="titulo">
                <?php
                // Obtener la sección y decodificar cualquier entidad HTML
                $seccion = htmlspecialchars_decode($seccion);

                // Convertir la primera letra a mayúscula y asegurarse de que el texto esté en la forma correcta
                $seccion = ucfirst($seccion);

                // Verificar si la sección es "Política" y ajustar si es necesario
                if (strtolower($seccion) === 'politica') {
                    $seccion = 'Política';
                }
                ?>
                <h1><?php echo htmlspecialchars($seccion); ?></h1>
            </div>

            <div class="portada">
                <div class="nota_principal">
                    <?php foreach ($conexion->query("SELECT * FROM noticias WHERE segment = '$seccion' AND priority_segment='primary' ORDER BY updated_at DESC LIMIT 1") as $noticia ): ?>
                        <a href="notas/nota.php?id_noticia=<?php echo $noticia['id']; ?>">
                            <div class="card text-bg-dark">
                                <img src="img/<?php echo $noticia['imagen']; ?>" class="card-img" alt="<?php echo $noticia['titulo']; ?>">
                ...
                <div class="notas_secundarias">
                    <?php foreach ($conexion->query("SELECT * FROM noticias WHERE segment = '$seccion' AND priority_segment='secondary' ORDER BY updated_at DESC LIMIT 2") as $noticia ): ?>
                        <a href="notas/nota.php?id_noticia=<?php echo $noticia['id']; ?>">
                            <div class="card border-1 border-top-0 border-end-0 border-start-0">
                                <img src="img/<?php echo $noticia['imagen']; ?>" class="card-img-top" alt="<?php echo $noticia['titulo']; ?>">
                                <div class="card-body">
            ...
            <div id="seg_notas" class="hero">
                <?php foreach ($conexion->query("SELECT * FROM noticias WHERE segment = '$seccion' AND priority_segment = 'secondary' ORDER BY updated_at DESC LIMIT 1000 OFFSET 2") as $noticia ): ?>
